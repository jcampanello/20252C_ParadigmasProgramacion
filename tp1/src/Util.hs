...
{-
    IDEA:   la idea es procesar la lista con foldr, generando una tupla como resultado. Al comenzar, el primer
            ...
            el primer espacio del segundo elemento (salvo que este seea cadena vacia).
...
{-
    IDEA:   juntar dos listas (la primera los valores y la segunda la secuencia de indices comenzando en cero)
            para poder decidir si N es el indice correcto y asi aplicar la funcion sobre el valor, caso contrario
            retornar el valor sin cambios.
            Notar que esto protege de indices menores a cero (la segunda lista nunca va a matchear) y de indices
            fuera del rango superior (no habra mas valores)
-}
